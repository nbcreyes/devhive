...
const LANGUAGES = [
  ...
  'kotlin', 'html', 'css', 'sql', 'bash', 'json', 'markdown',
];
...
function CodeEditor({ channelId }) {
  ...
  async function handleRunCode() {
    ...
    try {
      ...
      setOutput({ output: '', stderr: err.response?.data?.error || 'Execution failed', exitCode: 1 });
    } finally {
      setIsRunning(false);
    }
  }
    ...
    <div style={{ height: '100%', width: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
          ...
          {/* Output panel */}
          {showOutput && (
            <div className="border-t bg-zinc-900 text-zinc-100" style={{ height: '180px', display: 'flex', flexDirection: 'column' }}>
              <div className="flex items-center justify-between px-3 py-1 border-b border-zinc-700 shrink-0">
                ...
                <div className="flex items-center gap-2">
                  {output && (
                    <span className={cn('text-xs', output.exitCode === 0 ? 'text-green-400' : 'text-red-400')}>
                      Exit code: {output.exitCode}
                    </span>
                  )}
                  <button onClick={() => setShowOutput(false)} className="text-zinc-400 hover:text-zinc-100">
              ...
              <div className="flex-1 overflow-y-auto p-3">
                ...
                {output && (
                  <pre className="text-xs whitespace-pre-wrap font-mono">
                    {output.output && <span className="text-zinc-100">{output.output}</span>}
